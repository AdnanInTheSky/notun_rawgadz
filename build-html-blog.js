...
function buildHtmlBlog() {
  ...
  const generateHTML = (blog) => {
    const tagArray = blog.tags ? blog.tags.split(',').map(t => t.trim()).filter(Boolean) : [];
    const tagsHtml = tagArray.map(tag => `<span class="bg-neutral-100 text-neutral-800 text-[11px] font-bold px-3 py-1 rounded-full uppercase tracking-wider border border-neutral-200">${escapeHtml(tag)}</span>`).join('\n            ');
...
<head>
  ...
  <style>
    ...
    .blog-body pre { background-color: #171717; color: #f5f5f5; padding: 1.25rem; overflow-x: auto; margin-top: 1.5rem; margin-bottom: 1.5rem; font-size: 0.875rem; border-radius: 0.75rem; }
  ...
  <header class="sticky top-0 z-40 bg-white border-b border-neutral-200 shadow-sm w-full">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
      ...
      <nav class="hidden md:flex md:flex-row md:items-center md:gap-6 flex-grow">
        ...
        <a href="../blog.html" class="hover:text-black transition-colors font-bold text-xs tracking-wider uppercase text-black border-b-2 border-black pb-0.5">Blog</a>
    ...
    <article class="bg-white rounded-3xl border border-neutral-200 overflow-hidden w-full max-w-4xl mx-auto shadow-sm p-6 sm:p-10 md:p-14">
      ...
      ${blog.imageSrc ? `
        <div class="mb-10 rounded-2xl overflow-hidden bg-neutral-100 border border-neutral-200">
          <img src="${blog.imageSrc}" alt="${escapeHtml(blog.title)}" class="w-full max-h-[450px] object-cover">
        </div>
      ` : ''}
