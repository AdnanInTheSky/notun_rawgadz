...
class MyCheckout extends LitElement {
  ...
  async _handleSubmit(e) {
    ...
    if (!this.apiEndpoint || this.apiEndpoint.includes('script.google.com')) {
      this._showMessage('Update your API Endpoint to the Vercel backend.', false);
      return;
    }
    ...
    const orderTotal = cartArray.reduce((sum, item) => sum + (item.price * item.quantity), 0);
    ...
    try {
      ...
      if (response.ok && data.payment_url) {
        ...
        this._showMessage(data.error || 'Failed to initiate payment.', false);
      }
    } catch (err) {
